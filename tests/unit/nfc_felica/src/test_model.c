...
static felica_data_t s_copy; /* Static to avoid large stack allocation */
...
ZTEST(felica_model, test_serialize_roundtrip_golden)
{
	...
	ret = felica_deserialize(&s_copy, s_out, out_len);
	zassert_ok(ret);
	zassert_mem_equal(s_copy.idm, s_data.idm, FELICA_IDM_SIZE);
	zassert_mem_equal(s_copy.blocks[14].data, s_data.blocks[14].data, FELICA_BLOCK_DATA_SIZE);
}
