/*
 * Copyright (c) 2026
 * SPDX-License-Identifier: Apache-2.0
 *
 * Classic Tier fixtures — Flipper generator goldens (all shipped variants).
 */

#ifndef TESTS_FIXTURES_CLASSIC_CLASSIC_FIXTURE_H_
#define TESTS_FIXTURES_CLASSIC_CLASSIC_FIXTURE_H_

#include "MfClassic_1K_4b_mock.h"
#include "MfClassic_1K_7b_mock.h"
#include "MfClassic_4K_4b_mock.h"
#include "MfClassic_Mini_4b_mock.h"

#endif /* TESTS_FIXTURES_CLASSIC_CLASSIC_FIXTURE_H_ */
