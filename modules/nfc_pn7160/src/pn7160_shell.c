...
static int cmd_pn7160_settings(const struct shell *sh, size_t argc, char **argv)
{
	const struct device *dev;
	int ret;
	...
	ret = pn7160_nci_configure_settings(dev);
	if (ret != 0) {
		shell_error(sh, "ConfigureSettings failed: %d", ret);
		return ret;
	}

	shell_print(sh, "NCI settings applied (Nfc_settings.h blobs)");
	return 0;
}
