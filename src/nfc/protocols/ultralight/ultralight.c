...
void ultralight_config_page_indices(uint16_t pages_total, uint16_t *auth0_page,
				    ...
				    uint16_t *pack_page)
{
	if (pages_total < ULTRALIGHT_CONFIG_BLOCK_PAGES) {
		if (auth0_page != NULL) {
			*auth0_page = 0U;
		}
		if (access_page != NULL) {
	...
	if (pack_page != NULL) {
		*pack_page = (uint16_t)(pages_total - 1U);
	}
	if (pwd_page != NULL) {
		*pwd_page = (uint16_t)(pages_total - 2U);
	}
	if (access_page != NULL) {
		*access_page = (uint16_t)(pages_total - 3U);
	}
	if (auth0_page != NULL) {
		*auth0_page = (uint16_t)(pages_total - 4U);
	}
}
...
bool ultralight_parse_config(const ultralight_data_t *data, ultralight_config_t *cfg)
{
	...
	if (!ultralight_type_has_password_config(data->type)) {
		return false;
	}

	if (data->pages_total < ULTRALIGHT_CONFIG_BLOCK_PAGES) {
		return false;
	}

	ultralight_config_page_indices(data->pages_total, &auth0_page, &access_page, &pwd_page,
				       &pack_page);

	if (data->pages_read <= auth0_page) {
		return false;
	}
...
bool ultralight_read_protection_enabled(const ultralight_config_t *cfg)
{
	...
	return cfg->auth0 != ULTRALIGHT_AUTH0_NONE;
}
...
bool ultralight_page_needs_auth(const ultralight_config_t *cfg, uint16_t page)
{
	...
	return page >= (uint16_t)cfg->auth0;
}
